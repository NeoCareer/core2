#include "core2/async/AtomicQueue.h"
#include "core2/Bit.h"
...
TEST(AtomicQueueTest, AtomicQueueTestCorrectness) {
  core2::AtomicQueue<core2::u64> atomicQueue;
  ...
  const auto poper = [&]() {
    ...
    while (true) {
      try {
        core2::u64 value = atomicQueue.pop();
        popedSum += value;
      } catch (...) {
        if (failureLimit-- == 0) {
          break;
