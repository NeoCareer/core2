...
TEST(VectorTest, SimpleFifo) {
  ...
  const auto enqueueThread = [&]() {
    size_t inputIndex = 0;
    core::byte_t inBuffer[128];

    while (inputIndex < ITERATION) {
      size_t i;
      for (i = 0; i + inputIndex < ITERATION && i < 128; ++i) {
        inBuffer[i] = static_cast<core::byte_t>((i + inputIndex) * 117ull % 91ull);
      }
  ...
  const auto dequeueThread = [&]() {
    ...
    core::byte_t outBuffer[128];

    while (outputIndex < ITERATION) {
      size_t size = fifo.dequeue(outBuffer, 128);

      for (size_t i = 0; i < size; ++i) {
        ASSERT_EQ(static_cast<core::byte_t>((outputIndex + i) * 117ull % 91ull),
                  outBuffer[i]);
      }
